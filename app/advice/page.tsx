    ...
    <Layout>
      ...
      <section className="max-w-4xl mx-auto px-4 py-8 sm:px-6">
        ...
        <div className="border border-ink/10 rounded-xl shadow-hard bg-paper overflow-hidden flex flex-col h-[520px]">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {message.products.map((p) => (
                              <Link
                                ...
                                href={`/products/${p.id}`}
                                target="_blank"
                                className="flex items-center gap-2 bg-paper border border-ink/10 rounded-xl p-1.5 pr-3 hover:bg-ink/5 transition-colors max-w-[210px]"
                              >
          ...
          <div className="p-3 border-t border-ink/[0.07] bg-paper">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2.5 -mx-1 px-1 scrollbar-none">
                {FOLLOW_UPS.map((q) => (
                  <button
                    ...
                    className="flex-shrink-0 px-3 py-1.5 bg-paper border border-ink/10 rounded-full text-[11px] font-semibold text-ink hover:bg-ink hover:text-paper transition-colors"
                  >
                    {q}
                  </button>
                ))}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-ink text-paper px-6 text-sm font-semibold border border-ink/10 rounded-full hover:bg-accent transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Enviar
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {suggestedQuestions.map((q, idx) => (
              <button
                ...
                onClick={() => executeSendMessage(q)}
                disabled={loading}
                className="p-3.5 bg-paper border border-ink/10 rounded-xl shadow-hard hover-shift text-xs text-ink font-bold uppercase tracking-tight text-left flex justify-between items-center gap-2 disabled:opacity-50"
              >
                <span>{q}</span>
