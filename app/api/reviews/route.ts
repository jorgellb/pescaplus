...
export async function POST(request: NextRequest) {
  const limit = rateLimit(`review:${clientIp(request)}`, 20, 60 * 60_000)
  ...
  // Solo clientes reales (reserva pagada/aceptada) y solo salidas ya pasadas.
  ...
  try {
    const review = await createReview({ operatorId: charter.operatorId, charterId: charter.id, authorUserId: user.id, rating: parsed.data.rating, text: parsed.data.text })
