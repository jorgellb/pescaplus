  ...
  if (done && !open) {
    return (
      <span className="inline-flex flex-wrap items-center gap-2">
        <button onClick={() => setOpen(true)} className="text-[12px] font-bold uppercase tracking-wide text-ink/50 hover:text-accent">✓ Valorado · editar</button>
  ...
  return (
    <div className="mt-2 border-t border-ink/10 pt-3 space-y-2">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => setOpen(false)} className="px-4 py-2 text-sm font-semibold text-ink/50 hover:text-ink">Cancelar</button>
      </div>
    </div>
  )
}
