...
/** Rate the patrón after a trip. Shown on a past paid/accepted booking. */
export default function ReviewForm({ charterId, initialRating = 0, initialText = '', done: alreadyDone = false }: {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch('/api/reviews', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ charterId, rating, text }) })
  ...
  if (!open) {
    return <button onClick={() => setOpen(true)} className="text-[12px] font-bold uppercase tracking-wide text-accent hover:underline">★ Valorar al patrón</button>
  }
  return (
    <div className="mt-2 border-t border-ink/10 pt-3 space-y-2">
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value)} maxLength={800} rows={2} placeholder="¿Qué tal la salida? (opcional)" className="w-full border border-ink/20 rounded-xl bg-paper px-3 py-2 text-sm" />
