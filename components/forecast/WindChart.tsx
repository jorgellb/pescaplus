...
export default function WindChart({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" role="img" aria-label="Previsión de viento por horas">
      ...
      {/* solunar bands */}
      {periods.map((p, i) => (
        <rect key={i} x={x(Math.max(p.start, dayStart))} y={padT} width={Math.max(0, x(Math.min(p.end, dayEnd)) - x(Math.max(p.start, dayStart)))} height={innerH} fill="#0a7d72" opacity={0.08} />
      ))}
      ...
      {now >= dayStart && now <= dayEnd && (
        <line x1={x(now)} x2={x(now)} y1={padT} y2={padT + innerH} stroke="#0a7d72" strokeWidth={1.5} strokeDasharray="3 3" />
      )}
      {/* hour axis */}
