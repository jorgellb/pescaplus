...
export function buildTimeline(opts: {
  ...
  if (inDay(sol.firstLight)) items.push({ time: sol.firstLight, icon: 'sunrise', title: 'Primera luz', note: 'Empieza la ventana clásica del alba: los depredadores cazan con poca luz.', highlight: false })
  if (inDay(sol.sunrise)) items.push({ time: sol.sunrise, icon: 'sunrise', title: 'Amanecer', note: 'Hora prime: pesca activa ya montado desde la primera luz.', highlight: false })
  ...
  if (win) {
    items.push({ time: Math.max(win.start, dayStart), icon: 'target', title: `Mejor ventana del día (${win.avg} pts)`, note: 'El tramo con más probabilidad: prioriza estar pescando aquí.', highlight: true })
  }
  if (inDay(sol.sunset)) items.push({ time: sol.sunset, icon: 'sunset', title: 'Atardecer', note: 'Segunda hora prime del día; apura hasta la última luz.', highlight: false })
  if (inDay(sol.lastLight)) items.push({ time: sol.lastLight, icon: 'moon', title: 'Última luz', note: 'A partir de aquí, frontal y señuelos oscuros o luminosos.', highlight: false })
