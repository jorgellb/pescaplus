...
async function run() {
  ...
  try {
    ...
    for (const p of products) {
      await client.query(
        `INSERT INTO "Product"
          (id,"aliexpressId",title,description,"imageUrl",images,"videoUrl",price,currency,"affiliateUrl",category,"typeFishing",rating,reviews,"inStock","seoDescription","aiOptimized","updatedAt")
        ...
        [
          ...
          p.inStock ?? true, p.seoDescription ?? '', p.aiOptimized ?? false,
        ],
      )
    }
